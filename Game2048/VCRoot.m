...
- (void)viewDidLoad
{
    ...
    scoreLabel.frame = CGRectMake(10, 40, 300, 40);
...
//    _MAXScore.text = @"0";
    ...
    _bgView.frame = CGRectMake(5, 100, 310, 310);
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            GKLabel *label = [[GKLabel alloc] init];
            label.frame = CGRectMake(10+j*75, 10+i*75, 65, 65);
    ...
    startBtn.frame = CGRectMake(110, 425, 100, 40);
...
- (void)swAct:(UISwipeGestureRecognizer *)sw
{
    ...
    for (int i = 1; i <= 16; i++) {
        GKLabel *label = (GKLabel *)[self.view viewWithTag:i];
        if (label.text!=nil) {
            switch (label.num) {
            ...
            label.backgroundColor = [UIColor colorWithWhite:0.746 alpha:1.000];
        if (label.text != nil && label.num>maxScore) {
            maxScore = label.num;
        }
    }
...
- (void)btnStart
{
    if (isStart == NO) {
        ...
        GKLabel *label = (GKLabel *)[self.view viewWithTag:arc4random()%16+1];
        label.text = @"2";
        [self putNum];
        isStart = YES;
        for (int i = 1; i <= 16; i++) {
...
{
    for (int i = 0; i < 16; i++) {
        GKLabel *label = (GKLabel *)[self.view viewWithTag:i+1];
        if (label.text == nil) {
    ...
    GKLabel *label = _arrayLabels[index];
    int a = arc4random();
    if (a%8>6) {
        label.text = @"8";
    } else if (a > 4) {
    ...
    if (_arrayLabels.count == 1) {
        ...
        for (int i = 0; i < 4; i++) {
            int temp = 0;
            for (int j = 0; j < 4; j++) {
                GKLabel *label = (GKLabel *)[self.view viewWithTag:1+4*i+j];
                if (label.num == temp) {
        ...
        for (int i = 0; i < 4; i++) {
            int temp = 0;
            for (int j = 0; j < 4; j++) {
                GKLabel *label = (GKLabel *)[self.view viewWithTag:1+i+4*j];
                if (label.num == temp) {
                    [_arrayLabels removeAllObjects];
                    return;
                }
...
- (void)right
{
    ...
    for (int i = 0; i < 4; i++)
    {
        for (int j = 3; j > 0; j--)
        {
            GKLabel *label = (GKLabel *)[self.view viewWithTag:1+4*i+j];//取第一个
            NSLog(@"label(%d) = %@",label.tag,label.text);
            if (label.text != nil)
            {
                //第一个不为空取第二个
                GKLabel *label1 = (GKLabel *)[self.view viewWithTag:1+4*i+j-1];
                NSLog(@"label1(%d) = %@",label1.tag,label1.text);
                if (label1.text != nil) {
                    //第二个也不为空比较
                    if (label.num == label1.num)
                    {
                        label.text = [NSString stringWithFormat:@"%d",label.num*2];
...
- (void)moveRight
{
    for (int i = 0; i < 4; i++)
    {
        ...
        for (int j = 3; j >= 0; j--)
        {
            ...
            if (label.text != nil)
            {
                GKLabel *label1 = (GKLabel *)[self.view viewWithTag:index];
                if (label != label1)
                {
                    label1.text = [NSString stringWithFormat:@"%d",label.num];
                    label.text = nil;
                    isChangeR = YES;
                }
                index--;
...
- (void)up
{
    ...
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 3; j++)
        {
            GKLabel *label = (GKLabel *)[self.view viewWithTag:1+i+4*j];//取第一个
            NSLog(@"label(%d) = %@",label.tag,label.text);
            if (label.text != nil)
            {
                //第一个不为空取第二个
                GKLabel *label1 = (GKLabel *)[self.view viewWithTag:1+i+4*j+4];
                NSLog(@"label1(%d) = %@",label1.tag,label1.text);
                if (label1.text != nil) {
                    //第二个也不为空比较
                    if (label.num == label1.num)
